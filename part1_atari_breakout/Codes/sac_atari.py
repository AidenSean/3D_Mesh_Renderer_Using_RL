...
class SoftQNetwork(nn.Module):
    def __init__(self, envs):
        ...
        with torch.no_grad():
            out_dim = self.conv(torch.zeros(1, *obs_shape)).shape[1]

        self.fc = layer_init(nn.Linear(out_dim, 512))
        self.q = layer_init(nn.Linear(512, envs.single_action_space.n))
...
class Actor(nn.Module):
    def __init__(self, envs):
        ...
        self.fc = layer_init(nn.Linear(out_dim, 512))
        self.logits = layer_init(nn.Linear(512, envs.single_action_space.n))
    ...
    def get_action(self, x):
        ...
        action = dist.sample()
        return action, F.log_softmax(logits, dim=1), dist.probs
...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        rb.add(obs, next_obs, actions, rewards, terms, infos)
        obs = next_obs
    ...
    if args.save_model:
        ...
        torch.save(actor.state_dict(), f"{save_dir}/actor.pt")
        torch.save(qf1.state_dict(), f"{save_dir}/qf1.pt")
        torch.save(qf2.state_dict(), f"{save_dir}/qf2.pt")

        eval_env = make_env(
            args.env_id, args.seed, 0, True, f"{run_name}-eval"
        )()
        ...
        for ep in range(args.eval_episodes):
            ...
            while not done:
                ...
                obs, r, t, tr, _ = eval_env.step(action.item())
                total_r += r
                done = t or tr

            writer.add_scalar("eval/episodic_return", total_r, ep)
